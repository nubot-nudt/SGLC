...
{
    std::string conf_file = "../config/config_apollo.yaml";
    if (argc > 1)
    {
        conf_file = argv[1];
    }
    ...
    while (1)
    {
        ...
        if(frame_idx>100){
            ...
            for(int m=0;m<candidate_idx_dis.size();++m){
                ...
                double score = 1;
                if(candidate_idx_dis[m].second<0.15) score=SemGraph.loop_pairs_similarity(cloud_file,cloud_file2,sem_file,sem_file2);
                if(score==0){
